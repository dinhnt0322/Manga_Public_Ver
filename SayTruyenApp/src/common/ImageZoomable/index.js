...
class ZoomableImage extends Component {
  ...
  processTouch(x, y) {
    if (!this.state.isMoving) {
      ...
      this.setState({
        left:
          left > 0 ? 0 : maxOffset(left, this.state.width, this.props.imageWidth * this.state.zoom),
        top:
          top > 0 ? 0 : maxOffset(top, this.state.height, this.props.imageHeight * this.state.zoom),
      });
    }
  }
  ...
  render() {
    return (
      <View {...this._panResponder.panHandlers} onLayout={this._onLayout}>
        <Image
          {...this.props}
          style={{
            position: 'absolute',
